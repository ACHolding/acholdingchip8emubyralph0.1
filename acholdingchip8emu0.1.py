#!/usr/bin/env python3
"""AC's CHIP 8 EMU - chip8byralph.py

CHIP-8 emulator with an mGBA-style GUI: top menu bar, framed display,
status bar, black background, blue text on buttons.
...
def build_demo_rom() -> bytes:
    ...
    return code + bytes(padding_len) + ac_sprites
...
class Chip8App:
    ...
    def _build_menu(self) -> None:
        ...
        av_menu.add_command(label="Beep test", command=lambda: self.root.bell())
    ...
    def load_demo(self) -> None:
        ...
        self.running = True
        self.status_var.set("Loaded built-in AC demo")
        self.root.focus_set()
    ...
    def _show_about(self) -> None:
        messagebox.showinfo(TITLE, f"{TITLE}\n\nCHIP-8 emulator with mGBA-style GUI.")
    ...
    def _tick(self) -> None:
        if self.running and not self.chip8.halted:
            ...
            if self.chip8.sound_timer > 0:
                self.sound_pulse = (self.sound_pulse + 1) % 8
                if self.sound_pulse == 0:
                    self.root.bell()
            else:
                self.sound_pulse = 0
        ...
        self.fps_var.set("60 fps" if self.running else "-- fps")
        self.root.after(FRAME_MS, self._tick)
